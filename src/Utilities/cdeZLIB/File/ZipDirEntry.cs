...
// ------------------------------------------------------------------
//
// SPDX-FileCopyrightText: Copyright (c) 2006-2011 Dino Chiesa .
// All rights reserved.
//
    ...
    partial class ZipEntry
    {
            ...
            get
            {
                ...
                if (this._RelativeOffsetOfLocalHeader > 0xFFFFFFFF)
                    ...
                    .Append(string.Format("          Timeblob: 0x{0:X8}\n", this._TimeBlob))
                        .Append(string.Format("              Time: {0}\n", CDEngine.CDUtils.Zlib.SharedUtilities.PackedToDateTime(this._TimeBlob)));
                                              ...
                                              Dictionary<String,Object> previouslySeen)
        {
            ...
            if (IsNotValidZipDirEntrySig(signature))
            {
                ...
                {
                    throw new Exception(String.Format("  Bad signature (0x{0:X8}) at position 0x{1:X8}", signature, s.Position));
                }
                return null;
            }
            ...
            unchecked
            {
                ...
                zde._LastModified = CDEngine.CDUtils.Zlib.SharedUtilities.PackedToDateTime(zde._TimeBlob);
            ...
            {
                // UTF-8 is in use
                zde._FileNameInArchive = CDEngine.CDUtils.Zlib.SharedUtilities.Utf8StringFromBuffer(block);
            }
            else
            {
                zde._FileNameInArchive = CDEngine.CDUtils.Zlib.SharedUtilities.StringFromBuffer(block, expectedEncoding);
            }
            ...
            if (zde._commentLength > 0)
            {
                ...
                {
                    // UTF-8 is in use
                    zde._Comment = CDEngine.CDUtils.Zlib.SharedUtilities.Utf8StringFromBuffer(block);
                }
                else
                {
                    zde._Comment = CDEngine.CDUtils.Zlib.SharedUtilities.StringFromBuffer(block, expectedEncoding);
                }
            }
