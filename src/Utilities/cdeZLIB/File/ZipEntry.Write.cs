...
// ------------------------------------------------------------------
//
// SPDX-FileCopyrightText: Copyright (c) 2009-2011 Dino Chiesa
// All rights reserved.
//
...
namespace CDEngine.CDUtils.Zlib
{
    public partial class ZipEntry
    {
        ...
        private bool WantReadAgain()
        {
            ...
            if (CompressionLevel == CDEngine.CDUtils.Zlib.CompressionLevel.None) return false;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            if (CompressionLevel == (short)CDEngine.CDUtils.Zlib.CompressionLevel.None &&
                CompressionMethod == CDEngine.CDUtils.Zlib.CompressionMethod.Deflate)
                _CompressionMethod = 0x00;
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            // LastMod
            _TimeBlob = CDEngine.CDUtils.Zlib.SharedUtilities.DateTimeToPacked(LastModified);
            ...
            if (_container.Zip64 == Zip64Option.Never && (uint)_RelativeOffsetOfLocalHeader >= 0xFFFFFFFF)
                throw new Exception("Offset within the zip archive exceeds 0xFFFFFFFF. Consider setting the UseZip64WhenSaving property on the ZipFile instance.");
        ...
        private Int32 FigureCrc32()
        {
            if (_crcCalculated == false)
            {
                ...
                {
                    var output = new CDEngine.CDUtils.Zlib.CrcCalculatorStream(Stream.Null);
                ...
                else
                {
                    ...
                    var crc32 = new CDEngine.CDUtils.Zlib.CRC32();
        ...
        {
            if (_sourceStream == null)
                throw new Exception(String.Format("The input stream is null for entry '{0}'.", FileName));
            ...
            else if (this.Encryption == EncryptionAlgorithm.PkzipWeak)
            {
                ...
                if (this._Source != ZipEntrySource.ZipFile && ((this._BitField & 0x0008) != 0x0008))
                    throw new Exception("It is not possible to use PKZIP encryption on a non-seekable input stream");
            }
        }
        ...
        private void _WriteEntryData(Stream s)
        {
            ...
            try
            {
                ...
                var output = new CDEngine.CDUtils.Zlib.CrcCalculatorStream(compressor, true);
        ...
        internal void FinishOutputStream(Stream s,
                                         ...
                                         Stream encryptor,
                                         Stream compressor,
                                         CDEngine.CDUtils.Zlib.CrcCalculatorStream output)
        {
            ...
            if ((compressor as CDEngine.CDUtils.Zlib.DeflateStream) != null)
                compressor.Close();
#if BZIP
...
#if !NETCF
            else if ((compressor as CDEngine.CDUtils.Zlib.ParallelDeflateOutputStream) != null)
                compressor.Close();
#endif
            ...
            if (_UncompressedSize == 0 && _CompressedSize == 0)
            {
                ...
                if (_Password != null)
                {
                    ...
                    if (this._Source == ZipEntrySource.ZipOutputStream && !s.CanSeek)
                        throw new Exception("Zero bytes written, encryption in use, and non-seekable output.");
                    ...
                    if (Encryption != EncryptionAlgorithm.None)
                    {
                        ...
                        // workitem 10178
                        CDEngine.CDUtils.Zlib.SharedUtilities.Workaround_Ladybug318918(s);
        ...
        private void SetZip64Flags()
        {
            ...
            if (_container.Zip64 == Zip64Option.Never && _entryRequiresZip64.Value)
                throw new Exception("Compressed or Uncompressed size, or offset exceeds the maximum value. Consider setting the UseZip64WhenSaving property on the ZipFile instance.");
        ...
        internal void PrepOutputStream(Stream s,
                                       ...
                                       out CDEngine.CDUtils.Zlib.CrcCalculatorStream output)
        {
            ...
            // Wrap a CrcCalculatorStream around that.
            // This will happen BEFORE compression (if any) as we write data out.
            output = new CDEngine.CDUtils.Zlib.CrcCalculatorStream(compressor, true);
        }
        ...
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != CDEngine.CDUtils.Zlib.CompressionLevel.None)
            {
#if !NETCF
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    if (_container.ParallelDeflater == null)
                    {
                        _container.ParallelDeflater =
                            new CDEngine.CDUtils.Zlib.ParallelDeflateOutputStream(s,
                                                                       CompressionLevel,
                                                                       _container.Strategy,
                                                                       true);
                    ...
                    CDEngine.CDUtils.Zlib.ParallelDeflateOutputStream o1 = _container.ParallelDeflater;
                    o1.Reset(s);
                    return o1;
                }
#endif
                var o = new CDEngine.CDUtils.Zlib.DeflateStream(s, CDEngine.CDUtils.Zlib.CompressionMode.Compress,
                                                     CompressionLevel,
                                                     true);
                if (_container.CodecBufferSize > 0)
