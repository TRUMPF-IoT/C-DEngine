    ...
    public partial class ZipFile
    {
        ...
        public void Save()
        {
            try
            {
                ...
                if (WriteStream == null)
                    throw new Exception("You haven't specified where to save the zip.");

                if (_name != null && _name.EndsWith(".exe") && !_SavingSfx)
                    throw new Exception("You specified an EXE for a plain zip file.");
                ...
                if (_entries.Count >= 0xFFFF && _zip64 == Zip64Option.Never)
                    throw new Exception("The number of entries is 65535 or greater. Consider setting the UseZip64WhenSaving property on the ZipFile instance.");
        ...
        public void Save(String fileName)
        {
            ...
            if (Directory.Exists(_name))
                throw new Exception("Bad Directory", new System.ArgumentException("That name specifies an existing directory. Please specify a filename.", "fileName"));
            ...
            if (needZip64CentralDirectory)
            {
                if (zip64 == Zip64Option.Never)
                {
                    ...
                    if (sf.GetMethod().DeclaringType == typeof(ZipFile))
                        throw new Exception("The archive requires a ZIP64 Central Directory. Consider setting the ZipFile.UseZip64WhenSaving property.");
                    else
                        throw new Exception("The archive requires a ZIP64 Central Directory. Consider setting the ZipOutputStream.EnableZip64 property.");
#endif
