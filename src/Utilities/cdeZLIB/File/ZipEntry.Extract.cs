...
namespace CDEngine.CDUtils.Zlib
{
    ...
    public partial class ZipEntry
    {
        ...
        internal CDEngine.CDUtils.Zlib.CrcCalculatorStream OpenReader()
        {
            // workitem 10923
        ...
        /// <param name="password">The password to use for decrypting the entry.</param>
        /// <returns>The Stream for reading.</returns>
        internal CDEngine.CDUtils.Zlib.CrcCalculatorStream OpenReader(string password)
        {
            // workitem 10923
            if (_container.ZipFile == null)
                throw new InvalidOperationException("Use OpenReader() only with ZipFile.");
        ...
        internal CDEngine.CDUtils.Zlib.CrcCalculatorStream InternalOpenReader(string password)
        {
            ValidateCompression();
            ValidateEncryption();
            ...
            if (this._Source != ZipEntrySource.ZipFile)
                throw new Exception("You must call ZipFile.Save before calling OpenReader");
            ...
            // workitem 10178
            CDEngine.CDUtils.Zlib.SharedUtilities.Workaround_Ladybug318918(this.ArchiveStream);

            _inputDecryptorStream = GetExtractDecryptor(input);
            Stream input3 = GetExtractDecompressor(_inputDecryptorStream);

            return new CDEngine.CDUtils.Zlib.CrcCalculatorStream(input3, LeftToRead);
        }
        ...
        private void InternalExtract(string baseDir, Stream outstream, string password)
        {
            // workitem 7958
            if (_container == null)
                throw new Exception("This entry is an orphan");
            ...
            if (this._Source != ZipEntrySource.ZipFile)
                throw new Exception("You must call ZipFile.Save before calling any Extract method");
...
#else
            if (actualCrc32 != _Crc32)
                throw new Exception("CRC error: the file being extracted appears to be corrupted. " +
                                          String.Format("Expected 0x{0:X8}, Actual 0x{1:X8}", _Crc32, actualCrc32));
#endif
        }
            ...
            do
            {
                switch (ExtractExistingFile)
                {
                    ...
                    case ExtractExistingFileAction.InvokeExtractProgressEvent:
                        if (loop>0)
                            throw new Exception(String.Format("The file {0} already exists.", targetFileName));
        ...
        private Int32 ExtractOne(Stream output)
        {
            ...
            try
            {
                ...
                // workitem 10178
                CDEngine.CDUtils.Zlib.SharedUtilities.Workaround_Ladybug318918(input);
                ...
                using (var s1 = new CDEngine.CDUtils.Zlib.CrcCalculatorStream(input3))
                {
                    while (LeftToRead > 0)
                    {
        ...
        internal Stream GetExtractDecompressor(Stream input2)
        {
            ...
            switch (_CompressionMethod_FromZipFile)
            {
                ...
                case (short)CompressionMethod.Deflate:
                    return new CDEngine.CDUtils.Zlib.DeflateStream(input2, CDEngine.CDUtils.Zlib.CompressionMode.Decompress, true);
#if BZIP
        ...
        internal void _SetTimes(string fileOrDirectory, bool isFile)
        {
            ...
            try
            {
                ...
                {
                    // workitem 6191
                    DateTime AdjustedLastModified = CDEngine.CDUtils.Zlib.SharedUtilities.AdjustTime_Reverse(LastModified);
 ...
 Encryption != EncryptionAlgorithm.None)
            {
                ...
                if (_UnsupportedAlgorithmId != 0)
                    throw new Exception(String.Format("Cannot extract: Entry {0} is encrypted with an algorithm not supported by DotNetZip: {1}",
                                                         FileName, UnsupportedAlgorithm));
                else
                    throw new Exception(String.Format("Cannot extract: Entry {0} uses an unsupported encryption algorithm ({1:X2})",
        ...
        private void ValidateCompression()
        {
            if ((_CompressionMethod_FromZipFile != (short)CompressionMethod.None) &&
                ...
                throw new Exception(String.Format("Entry {0} uses an unsupported compression method (0x{1:X2}, {2})",
