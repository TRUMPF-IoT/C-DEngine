    ...
    public partial class ZipEntry
    {
        ...
        internal static int ReadWeakEncryptionHeader(Stream s, byte[] buffer)
        {
            ...
            if (additionalBytesRead != 12)
                throw new Exception(String.Format("Unexpected end of data at position 0x{0:X8}", s.Position));
        ...
        internal static void HandlePK00Prefix(Stream s)
        {
            ...
            uint datum = (uint)CDEngine.CDUtils.Zlib.SharedUtilities.ReadInt(s);
        ...
        private static void HandleUnexpectedDataDescriptor(ZipEntry entry)
        {
            ...
            {
                int sz = CDEngine.CDUtils.Zlib.SharedUtilities.ReadInt(s);
                if (sz == entry._CompressedSize)
                {
                    sz = CDEngine.CDUtils.Zlib.SharedUtilities.ReadInt(s);
        ...
        private int ProcessExtraFieldZip64(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize > 28)
                throw new Exception(String.Format("  Inconsistent size (0x{0:X4}) for ZIP64 extra field at position 0x{1:X16}",
                                                         dataSize, posn));
            int remainingData = dataSize;

            var slurp = new Func<Int64>( () => {
                    if (remainingData < 8)
                        throw new Exception(String.Format("  Missing data for ZIP64 extra field, position 0x{0:X16}", posn));
        ...
        private int ProcessExtraFieldInfoZipTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            if (dataSize != 12 && dataSize != 8)
                throw new Exception(String.Format("  Unexpected size (0x{0:X4}) for InfoZip v1 extra field at position 0x{1:X16}", dataSize, posn));
        ...
        private int ProcessExtraFieldUnixTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize != 13 && dataSize != 9 && dataSize != 5)
                throw new Exception(String.Format("  Unexpected size (0x{0:X4}) for Extended Timestamp extra field at position 0x{1:X16}", dataSize, posn));
        ...
        private int ProcessExtraFieldWindowsTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize != 32)
                throw new Exception(String.Format("  Unexpected size (0x{0:X4}) for NTFS times extra field at position 0x{1:X16}", dataSize, posn));
