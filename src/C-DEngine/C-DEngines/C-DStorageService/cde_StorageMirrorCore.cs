...
namespace nsCDEngine.Engines.StorageService
{
    ...
    public class TheMirrorCacheCore<T> : IDisposable where T : TheDataBase, INotifyPropertyChanged, new()
    {
        /// <summary>
...
#pragma warning restore S2583 // Conditionally executed code should be reachable
        ...
        /// </summary>
        internal int CacheCounter;
        /// <summary>
        /// KPI
        /// </summary>
        internal long TotalCacheCounter;
        internal DateTimeOffset LastCacheStoreTime = DateTimeOffset.MinValue;
        ...
        public TheMirrorCacheCore()
        {
            ...
            MyRecordsRWLock = new TheMirrorCacheReaderWriterLockCore<T>(this);
            IsReady = true;
        }
        /// <summary>
        /// Initializes a new instance of the <see cref="TheMirrorCache{T}"/> class.
        ...
        public TheMirrorCacheCore(int pExpirationTime)
        {
            MyStoreID = TheStorageUtilities.GenerateUniqueIDFromType(typeof(T), null);  //null is save...will be overwritten by MirrorCache with cachetablename
            MyRecordsRWLock = new TheMirrorCacheReaderWriterLockCore<T>(this);
        ...
        public bool SaveCacheToDisk(bool SaveSync, bool WaitForSave)
        {
            ...
            if (!SaveSync)
            {
                if (!IsStoreIntervalInSeconds && CacheCounter < CacheStoreInterval)
                {
                    return false;
                }
                if (!UseSafeSave)
        ...
        private void DoSaveCacheToDisk(bool waitForSave, bool bForce)
        {
            ...
            lock (saveLock) // Ensure we do at most one file write at a time (per cache)
            {
                ...
                var timeSinceLastSave = DateTimeOffset.Now - LastCacheStoreTime;
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        LastCacheStoreTime = DateTimeOffset.Now;
                        CacheCounter = 0;
                    }
        ...
        {
            List<T> retList = new();
            MyRecordsRWLock.RunUnderWriteLock(() =>  //LOCK-REVIEW: is this ok for consistency? -> Possible optimization: Could only use write lock around DoAddItem
            {
...
#pragma warning restore S2583 // Conditionally executed code should be reachable
        ...
        {
            if (MaxStoreSize == 0) return; //LOCK-REVIEW: We do not want to do this if there is a lock already here (logic rather then read/write) MUST NOT CHECK FOR:  || MyRecordsRWLock.IsLocked() <= thi is NEVER false!
    ...
    internal class TheMirrorCacheReaderWriterLockCore<T> : IReaderWriterLock where T : TheDataBase, INotifyPropertyChanged, new()
    {
        readonly ReaderWriterLockSlim _rwLock;
        readonly TheMirrorCacheCore<T> _cache;
        TimeSpan _timeout;
        bool _hasUsedUpgradebleMode;
        readonly bool DoLog = false;
        public TheMirrorCacheReaderWriterLockCore(TheMirrorCacheCore<T> cache)
        {
